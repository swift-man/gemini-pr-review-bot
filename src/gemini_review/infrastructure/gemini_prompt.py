...
def render_pr_conversation(conversation: PrConversation | None) -> str:
    ...
    출력 구조 (3 그룹):
      ...
      <안내 문구 — 같은 지적 반복 / deferred 항목 재flag 금지>

      --- REVIEWS (전체 리뷰 submission) ---
      [<timestamp>] <author> [bot|author] (<state>): <body>

      --- DISCUSSION (top-level 토론) ---
      [<timestamp>] <author> [bot|author]: <body>

      --- LINE COMMENTS ---
      [<timestamp>] <author> [bot|author] @ <path>:<line> (cid=<id>[, reply_to=<id>]): <body>

    cap 초과 시 오래된 entry 부터 잘라내고 "... (older N entries truncated)" 마커.
    """
    if conversation is None or conversation.is_empty():
        return ""

    # entries 는 시간 오름차순 정렬돼 있음. cap 적용을 위해 newest 부터 거꾸로 모아
    # 누적 char 가 한도 안에 들어가는 만큼만 채운다. 이후 다시 시간순으로 출력.
    by_kind = {"review": [], "issue_comment": [], "line_comment": []}
    rendered_entries: list[tuple[str, ConversationEntry]] = []
    for entry in conversation.entries:
        rendered = _format_conversation_entry(entry)
        rendered_entries.append((rendered, entry))

    # newest-first 누적
    selected_indices: list[int] = []
    total = 0
    for i in range(len(rendered_entries) - 1, -1, -1):
        rendered, _ = rendered_entries[i]
        # +1 은 항목 사이 newline 근사
        if total + len(rendered) + 1 > _CONVERSATION_MAX_CHARS and selected_indices:
            break
        total += len(rendered) + 1
        selected_indices.append(i)
    selected_indices.sort()  # 다시 시간순

    truncated = len(rendered_entries) - len(selected_indices)
    selected = [rendered_entries[i] for i in selected_indices]
    ...
    lines = [
        "=== PR CONVERSATION HISTORY ===",
        "(이전 라운드의 reviewer 의견 / 작성자 응답 — 같은 지적 반복 금지, 작성자가",
        " 'follow-up' / 'deferred' 로 분리한 항목 재flag 금지. 다른 봇 finding 은 컨텍스트",
        " 일 뿐 — 직접 코드 확인 후 독립 판정.)",
    ]
    if truncated > 0:
